...
class BaseNode:
    ...
    def has_started(self) -> bool:
        ...
        A failed node counts as started, since it must have started to fail.
    ...
    def _register_node(
        ...
        with self._lock:
            ...
            for start_upstream_id in start_upstream_ids or []:
                ...
                self._nodes[start_upstream_id].downstream_ids.add(node.node_id)
            ...
            if any(
                ...
                for upstream_id in node.upstream_ids | node.start_upstream_ids
            ):
                self._set_node_state(
    ...
    def _set_node_state(self, node_id: str, state: NodeState) -> bool:
        ...
        with self._lock:
            ...
            if node.state == NodeState.RUNNING:
                # Entering RUNNING can release nodes that only wait for this
                # node to start.
